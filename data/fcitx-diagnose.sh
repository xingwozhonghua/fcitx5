...
__fcitx_init() {
    if fcitx_exe="$(which fcitx5 2> /dev/null)"; then
        ...
        fcitx_ver=5
        return
    elif fcitx_exe="$(which fcitx 2> /dev/null)"; then
        fcitx_name=fcitx
        Fcitx_name=Fcitx
        fcitx_ver=4
        return
...
__conf_dir_init() {
    ...
    fx_conf_home=${_xdg_conf_home}/${fcitx_name}
...
check_env() {
    ...
    write_order_list_eval "$(_ '${1} Settings Directory:')" ${Fcitx_name}
    write_eval \
...
check_fcitx() {
    ...
    if [[ -z "$fcitx_exe" ]]; then
        ...
        write_eval "$(_ 'Found ${1} at ${2}.')" ${fcitx_name} "$(code_inline "${fcitx_exe}")"
    fi
    ...
    process=()
    while read line; do
        if [[ $line =~ ^([0-9]*)\ .*${fcitx_name}.* ]]; then
            [ "${BASH_REMATCH[1]}" = "$$" ] && continue
            array_push process "${line}"
        fi
    done <<< "${psoutput}"
    if ! ((${#process[@]})); then
        write_error_eval "$(_ '${1} is not running.')" ${Fcitx_name}
        __need_blank_line=0
        write_error_eval "$(_ 'Please check the Configure link of your distribution in ${1} for how to setup ${2} autostart.')" "$(beginner_guide_link)" ${fcitx_name}
        return 1
    fi
    local pcount="${#process[@]}"
    if ((pcount > 1)); then
        write_eval "$(_ 'Found ${1} ${2} processes:')" "${#process[@]}" ${fcitx_name}
    else
        write_eval "$(_ 'Found ${1} ${2} process:')" "${#process[@]}" ${fcitx_name}
    fi
    write_quote_cmd print_array "${process[@]}"
    write_order_list "$(code_inline "${fcitx_name}-remote"):"
    if type ${fcitx_name}-remote &> /dev/null; then
        if ! ${fcitx_name}-remote &> /dev/null; then
            write_error_eval "$(_ 'Cannot connect to ${1} correctly.')" ${fcitx_name}
        else
            write_eval "$(_ '${1} works properly.')" \
                       "$(code_inline "${fcitx_name}-remote")"
        fi
    else
        write_error "$(print_not_found "${fcitx_name}-remote")"
    fi
    write_order_list "$(_ "DBus interface:")"
    local dbus_name=org.fcitx.${Fcitx_name}
    if [[ -n ${dbus_exe} ]]; then
...
_find_config_gtk() {
    ...
    local config_gtk
    config_gtk="$(which "${fcitx_name}-config-gtk" 2> /dev/null)" || return 1
    echo "${config_gtk}"
...
_check_config_gtk() {
    ...
    if ! config_gtk="$(which "${fcitx_name}-config-gtk${version}" 2> /dev/null)"; then
        if ! _check_config_gtk_version "${version}"; then
            ...
            config_gtk_name="${fcitx_name}-config-gtk"
        fi
    else
        config_gtk_name="${fcitx_name}-config-gtk${version}"
    fi
...
_check_config_qt() {
    ...
    config_qt_name="${fcitx_name}-config-qt"
...
_check_config_kcm() {
    ...
    config_kcm="$(${kcm_shell} --list 2> /dev/null | grep -i ${fcitx_name})" && {
        write_eval "$(_ 'Found ${1} kcm module.')" ${fcitx_name}
...
check_config_ui() {
    ...
    if ! fcitx_configtool="$(which ${fcitx_name}-configtool 2> /dev/null)"; then
        write_error_eval "$(_ 'Cannot find ${1} executable!')" ${fcitx_name}-configtool
    else
        write_eval "$(_ 'Found ${1} at ${2}.')" \
                   ${fcitx_name}-configtool \
    ...
    if [[ $fcitx_ver = 4 ]]; then
        ...
        _check_config_kcm 4 && config_backend_found=1
...
check_qt() {
    ...
    for file in "${qt_modules[@]}"; do
        ...
        __need_blank_line=0
        if [[ ${basename} =~ im-${fcitx_name} ]] &&
            [[ ${file} =~ plugins/inputmethods ]]; then
            write_eval "$(_ 'Found ${3} im module for ${2}: ${1}.')" \
                "$(code_inline "${file}")" Qt4 ${fcitx_name}
            qt4_module_found=1
        elif [[ ${basename} =~ ${fcitx_name}platforminputcontextplugin ]] &&
            [[ ${file} =~ plugins/platforminputcontexts ]]; then
            write_eval "$(_ 'Found ${3} im module for ${2}: ${1}.')" \
                "$(code_inline "${file}")" Qt5 ${fcitx_name}
            qt5_module_found=1
        elif [[ ${file} =~ /${fcitx_name}/qt/ ]]; then
            write_eval "$(_ 'Found ${1} ${2} module: ${3}.')" \
                        ${fcitx_name} qt4 "$(code_inline "${file}")"
        elif [[ ${file} =~ /${fcitx_name}/qt5/ ]]; then
            write_eval "$(_ 'Found ${1} ${2} module: ${3}.')" \
                        ${fcitx_name} qt5 "$(code_inline "${file}")"
        else
    ...
    if [ -z "${qt4_module_found}" ]; then
        __need_blank_line=0
        write_error_eval \
            "$(_ 'Cannot find ${1} input method module for ${2}.')" ${fcitx_name} Qt4
    fi
    if [ -z "${qt5_module_found}" ]; then
        __need_blank_line=0
        write_error_eval \
            "$(_ 'Cannot find ${1} input method module for ${2}.')" ${fcitx_name} Qt5
    fi
}
...
check_gtk_immodule_cache() {
    ...
    for cache in "${gtk_immodules_cache[@]}"; do
        ...
        if fcitx_gtk=$(grep ${fcitx_name} <<< "${cache_content}"); then
...
check_modules() {
    ...
    addon_conf_dir="$(get_config_dir addonconfigdir addon)" || {
        write_error "$(_ 'Cannot find ${1} addon config directory.')" ${fcitx_name}
        return
    }
    local enabled_addon=()
    ...
    write_eval "$(_ 'Found ${1} addon config directory: ${2}.')" \
               ${fcitx_name} "$(code_inline "${addon_conf_dir}")"
    if [[ $fcitx_ver = 5 ]]; then
    ...
    if ! ((${#enabled_ui[@]})); then
        write_error_eval "$(_ 'Cannot find enabled ${1} user interface!')" ${fcitx_name}
    else
...
check_input_methods() {
    ...
    local imlist=($(get_from_config_file \
        ...
        write_error_eval "$(_ 'Cannot read im list from ${1} profile.')" ${fcitx_name}
        return 0
    }
    local enabled_im=()
    local disabled_im=()
...
check_log() {
    ...
    if [[ $fcitx_ver = 4 ]]; then
        ...
        logdir="${fx_conf_home}/log"
        pretty_logdir="${fx_conf_pretty_home}/log"
    else
        logdir="${fx_conf_home}"
